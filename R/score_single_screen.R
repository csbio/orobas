...
#' @param condition_screen_names Name of a condition screen to score against the 
#'   control screen.
...
{  
	...
	for (i in 1:n_genes) { # iterate over unique gene indexes
		...
		# Takes the mean across replicates for condition 
		...
		if (test == "rank-sum") {
			...
			condition_reps = condition_cols # get condition replicate names
	...
	if(test == "moderated-t"){
		...
		condition_residuals = jackknife_outliers(condition_residuals)
	...
	if (return_residuals & (loess & test == "moderated-t")) {		
		...
		cat("WARNING: returning residuals is currently only supported with loess-normalization and moderated-t test enabled\n")		
...
)
{
	...
	scores_fdr_fname <- file.path(parent_folder, "fdr_scores.tsv")
